                ...
                if not Motion.start_pick_up:
                    ...
                    if distance < 0.5:
                        ...
                        if time.time() - self.t1 > 1:
                            ...
                            Perception.world_X = Perception.world_X + 1
                            Perception.world_Y = Perception.world_Y + 0.5
                            Perception.count = 0
        ...
        while True:
            if self.isRunning:
                if Perception.detect_color != 'None' and Motion.start_pick_up:  # If it is detected that the block has not moved for a while, start the gripping
                    ...
                    self.set_buzzer(0.1)
                    result = AK.setPitchRangeMoving((Perception.world_X, Perception.world_Y, 8), -90, -90, 0)
                    if result == False:
